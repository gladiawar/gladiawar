...
public class Lobby : MonoBehaviour 
{
	...
	private enum State : int { none = 0, waiting, login_window, reg_window, recoverpw_window, inlobby }
		...
		{	
			...
			GUI.Box(new Rect(0, 0, 220, 150), "settings sample");
				...
				if (GUI.Button(new Rect(5, 100, 210, 30), "save settings"))
				{
					...
					GamePHP.Instance.opt_email_notify = (emailNotify ? 1 : 0);
					SDNet.Instance.PriorityRequest(GamePHP.Instance.MainUrl + "settings/", OnNetSettingsSaved, new Dictionary<string, string> { 
						{ "email_notify", GamePHP.Instance.opt_email_notify.ToString() },
					});
				}
