...
public class ChatGui: MonoBehaviour 
{
	...
	private Rect plrLstRect = new Rect(700, 50, 280, 245);
	...
	void LateUpdate()
	{
		...
		{
			if (message_refresh + GamePHP.Instance.ChatMsgRefreshTimeout < Time.time)
			{
				message_refresh = Time.time;
				SDNet.Instance.NormalRequest(GamePHP.Instance.MainUrl + "msgs/", OnNetChatMessages, null);
			}

			if (friends_refresh + GamePHP.FRIENDS_REFRESH_TIMEOUTS < Time.time)
			{
				RefreshFriendList(false);
			}

			if (players_refresh + GamePHP.FRIENDS_REFRESH_TIMEOUTS < Time.time)
			{
				RefreshPlayersList();
			}
			...
			{
				SDNet.Instance.NormalRequest(GamePHP.Instance.MainUrl + "cpm/", OnNetPmNames, new Dictionary<string, string> { 
	...
	private void GUI_EnterName(int id)
	{
		...
		if (GUI.Button(new Rect(125, 185, 100, 30), "SUBMIT"))
		{
			if (state == State.addfriend)
			{
				...
				state = State.waiting;
				SDNet.Instance.NormalRequest(GamePHP.Instance.MainUrl + "addf/", OnNetFriendAdded, new Dictionary<string, string> { { "u", nameInput } });
			}
			else if (state == State.contact)
			{
				winMsg = "Please wait. Creating private channel.";
				state = State.waiting;
				SDNet.Instance.NormalRequest(GamePHP.Instance.MainUrl + "cpm/", OnNetStartPm, new Dictionary<string, string> { { "u", nameInput } });
			}
		}
	}
	...
	private void GUI_PlayerList()
	{
		...
		if (GUI.Button(new Rect(plrLstRect.x + 245, 3, 50, 45), "del\nfriend"))
		{
			if (Chat.Instance.friendNames != null)
			{
				...
				state = State.waiting;
				SDNet.Instance.NormalRequest(GamePHP.Instance.MainUrl + "delf/", OnNetFriendDeleted, new Dictionary<string, string> { { "u", Chat.Instance.friends[selectedFriend].id.ToString() } });
			}
			else
			{
		...
		if (GUI.Button(new Rect(plrLstRect.x + 130, 3, 60, 45), "contact"))
		{
			...
			{
				if (Chat.Instance.playerNames[selectedPlayer].ToLower().Equals(GamePHP.Instance.name.ToLower()))
				{
					state = State.msg;
	...
	private void GUIChatSubmit()
	{
		...
		Chat.Instance.AddMessage(activeChannel, GamePHP.Instance.name + ": " + msg);
		...
		GamePHP.Instance.Update_ChatMsgRefreshTimeout(false); // dec wait time cause Im expecting a reply soon
		message_refresh = Time.time;
		SDNet.Instance.NormalRequest(GamePHP.Instance.MainUrl + "msgs/", OnNetChatMessages, new Dictionary<string, string> { 
	...
	private void RefreshFriendList(bool forced)
	{
		...
		if (has_friends || forced) // no reason to check if know the player dont have friends.. or when forced to check
			SDNet.Instance.NormalRequest(GamePHP.Instance.MainUrl + "listf/", OnNetFriendList, null);
	}

	private void RefreshPlayersList()
	{
		players_refresh = Time.time;
		SDNet.Instance.NormalRequest(GamePHP.Instance.MainUrl + "listp/", OnNetPlayerList, null);
	}
